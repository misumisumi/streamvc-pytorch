...
from streamvc.models.encoder.seanet import (
    SLSTM,
    SConv1d,
    SEANetResnetBlock,
...
from streamvc.modules.snake_activation import Snake1d
from streamvc.modules.film import FiLM
...
class SEANetDecoder(nn.Module):
    ...
    def __init__(
        ...
        if lstm:
            prenet += [SLSTM(mult * n_filters, num_layers=lstm)]
        ...
        for i, ratio in enumerate(self.ratios):
            ...
            for j in range(n_residual_layers):
                ...
                # apply FiLM between residual blocks
                if condition_dim > 0 and j != n_residual_layers - 1:
                    resblocks += [FiLM(mult * n_filters // 2, condition_dim)]

            mult //= 2
        self.resblocks = nn.Sequential(*resblocks)

        # Add final layers
        postnet: List[nn.Module] = [
            act(**activation_params),
            SConv1d(
                n_filters,
                channels,
        ...
        if final_activation is not None:
            ...
            postnet += [final_act(**final_activation_params)]
        self.postnet = nn.Sequential(*postnet)

    def forward(
        self, z: torch.Tensor, c: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        y = self.prenet(z)
